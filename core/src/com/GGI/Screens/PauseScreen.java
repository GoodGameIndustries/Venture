...
public class PauseScreen implements Screen, InputProcessor{
	...
	public PauseScreen(Venture g, GameScreen gs){
		this.g=g;
		this.gs=gs;
		m = g.assets.map;
	...
	public void render(float delta) {
		...
		for(int i = 0; i < m.map.length;i++){
			for(int j = 0; j < m.map[0].length;j++){
				if((m.x!=i||m.y!=j)&&m.map[i][j].state==1){
					pic.draw(g.assets.grey,(float) (.46f+((float)(i-m.x)/10))*w,(float) (.46f+((float)(j-m.y)/10))*h,.08f*w,.08f*h);
				}
				else if((m.x!=i||m.y!=j)&&m.map[i][j].state==2){
					pic.draw(g.assets.blue,(float) (.46f+((float)(i-m.x)/10))*w,(float) (.46f+((float)(j-m.y)/10))*h,.08f*w,.08f*h);
				}
				else{
					pic.draw(g.assets.gold,.46f*w,.46f*h,.08f*w,.08f*h);
				}
			}
		}
		pic.end();
